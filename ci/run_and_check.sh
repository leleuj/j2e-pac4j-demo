...
# Script to launch jee-pac4j-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting jee-pac4j-demo..."
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    if [ "$CAS_HTTP_CODE" = "200" ] && echo "$CAS_CONTENT" | grep -q "Enter Username & Password"; then
        ...
        if [ -z "$EXECUTION" ]; then
            ...
            # Go directly to index.jsp with authentication cookies
            INDEX_JSP_URL="http://localhost:8080/index.jsp"
            FINAL_META=$(curl -s -c "$COOKIE_JAR" -b "$COOKIE_JAR" -L -o "$FINAL_APP_PAGE" -w "FINAL_URL:%{url_effective}\nHTTP_CODE:%{http_code}" "$INDEX_JSP_URL")
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 jee-pac4j-demo test completed successfully!"
    ...
    echo "💥 jee-pac4j-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
